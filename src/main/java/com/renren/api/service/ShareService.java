...
public class ShareService {
    ...
    /**
     * 获取站内UGC分享数及浏览数  值为-1表示获取不到对应数据，TYPE_SHARE类型，不支持获取浏览数
     * @param ugcOwnerId 用户ID
     * @param ugcId 分享UGC的ID
     * @param ugcType 分享UGC类型
     * @return ShareViewCount 封装分享数及浏览数结果，值为-1表示获取不到对应数据
     * @throws   RennException 
     * SERVICE_UNAVAILABLE 服务端临时不可用 
     * UGC_OWNER_NOT_EXIST UGC所有者不存在 
     * UNSUPPORTED_UGC_TYPE 不支持的UGC_TYPE 
     * MISSING_UGC_ID UGC_ID不能为空 
     * MISSING_UGC_TYPE UGC_TYPE不能为空 
     */
    public ShareViewCount getShareCount (Long  ugcOwnerId, Long  ugcId, UGCType  ugcType) throws RennException {
        Map<String, String> textParams = new HashMap<String, String>();
        if (ugcOwnerId != null) {
            textParams.put("ugcOwnerId", RennParam.asString(ugcOwnerId));
        }
        ...
        Map<String, String> bodyParams = new HashMap<String, String>();
        Map<String, File[]> fileParams = new HashMap<String, File[]>();
        RennRequest request = new RennRequest("/v2/share/count/get", Method.GET, textParams, bodyParams, fileParams, token);
        RennResponse response = executor.execute(request);
        try {
            return (ShareViewCount)mapper.mapCommon(response.getResponse().toString(), ShareViewCount.class);
        } catch (JsonMappingException e) {
            throw new ObjectMappingException();
        }
    }
    ...
    /**
     * 获取视频、链接等源地址分享数及浏览数   值为-1表示获取不到对应数据
     * @param url 分享资源(视频、链接等)源地址
     * @return ShareViewCount 封装分享数及浏览数结果，值为-1表示获取不到对应数据
     * @throws   RennException 
     * SERVICE_UNAVAILABLE 服务端临时不可用 
     * URL_IS_ILLEGAL 非法的URL 
     * MISSING_URL 源地址URL不能为空 
     */
    public ShareViewCount getShareUrlCount (String  url) throws RennException {
        Map<String, String> textParams = new HashMap<String, String>();
        if (url != null) {
	    textParams.put("url", url);
        }
        Map<String, String> bodyParams = new HashMap<String, String>();
        Map<String, File[]> fileParams = new HashMap<String, File[]>();
        RennRequest request = new RennRequest("/v2/share/url/count/get", Method.GET, textParams, bodyParams, fileParams, token);
        RennResponse response = executor.execute(request);
        try {
            return (ShareViewCount)mapper.mapCommon(response.getResponse().toString(), ShareViewCount.class);
        } catch (JsonMappingException e) {
