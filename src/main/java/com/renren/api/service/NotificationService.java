...
public class NotificationService {
     ...
     * @param userIds 接收通知的用户列表。这些用户必须是当前登录用户的好友。请确保一次传入的用户id数少于20个。不能给当前用户发送通知，发送的通知会被忽略
     ...
     * @param apiKey 应用的标识：api_key
     * @return Integer 发送成功返回1
     * @throws   RennException 
     * APP_NOT_EXIST apiKey不属于任何应用。 
     * NOTIFICATION_TEXT_INVALID 通知内容不合法。 
     * NOTIFICATION_TEXT_PROHIBITED 抱歉，某些信息不能发布的哦：）谢谢您的谅解。 
     ...
     */
    public Integer putNotificationApp (String  content, Long [] userIds, String  apiKey) throws RennException {
        Map<String, String> textParams = new HashMap<String, String>();
        textParams.put("content", content);
        textParams.put("userIds", RennParam.asString(userIds));
        textParams.put("apiKey", apiKey);
        Map<String, String> bodyParams = new HashMap<String, String>();
        Map<String, File> fileParams = new HashMap<String, File>();
